...
import BatterySummary from '~/components/boxes/Batteries/BatterySummary'
...
const Batteries = ({ mode = 'compact' }: BoxProps) => {
  const { batteries } = useBattery()
  const router = useRouter()

  if (!batteries) {
    return <Box title={'Batteries'}>none</Box>
  }
  ...
  if (mode === 'compact') {
    return (
      <Box
        title={'Batteries'}
        onExpandClick={() => router.push(`${RouterPath.BOX}/Batteries`)}
      >
...
export default observer(Batteries)